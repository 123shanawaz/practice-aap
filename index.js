...
function handleSlider() {
  ...
  inputSlider.style.backgroundSize =
    ((passwordLength - mini) * 100) / (maxi - mini) + "% 100%";
}
...
function calculateStrength() {
  ...
  if (cnt > 2) {
    if (passwordLength >= 5) setIndicator("#ff0");
    else setIndicator("#f00");
  } else if (cnt == 2) {
    if (passwordLength >= 6) setIndicator("#ff0");
    else setIndicator("#f00");
  } else {
    if (passwordLength >= 8) setIndicator("#ff0");
    else setIndicator("#f00");
  }
}
...
copyBtn.addEventListener("click", () => {
  if (passwordDisplay.value.length) copyContent();
...
function handleCheckBoxChange() {
  ...
  if (passwordLength < checkCnt) passwordLength = checkCnt;
  handleSlider();
}
